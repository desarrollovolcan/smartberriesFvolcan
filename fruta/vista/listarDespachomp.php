...
function obtenerDatosCorreoDespacho($despacho, $PRODUCTOR_ADO, $PLANTA_ADO, $COMPRADOR_ADO, $EMPRESA_ADO, $TEMPORADA_ADO)
{
    ...
    switch ($despacho['TDESPACHO'] ?? '') {
        case '1':
            $tipoDespacho = 'Interplanta';
            $plantaDestino = $PLANTA_ADO->verPlanta($despacho['ID_PLANTA2'] ?? null);
            $destino = $plantaDestino ? ($plantaDestino[0]['NOMBRE_PLANTA'] ?? '') : '';
            break;
        case '2':
            $tipoDespacho = 'Devolución Productor';
            $productor = $PRODUCTOR_ADO->verProductor($despacho['ID_PRODUCTOR'] ?? null);
            $destino = $productor ? ($productor[0]['NOMBRE_PRODUCTOR'] ?? '') : '';
            break;
        case '3':
            $tipoDespacho = 'Venta';
            $comprador = $COMPRADOR_ADO->verComprador($despacho['ID_COMPRADOR'] ?? null);
            $destino = $comprador ? ($comprador[0]['NOMBRE_COMPRADOR'] ?? '') : '';
            break;
        case '4':
            $tipoDespacho = 'Despacho de Descarte(R)';
            $destino = $despacho['REGALO_DESPACHO'] ?? '';
            break;
        case '5':
            $tipoDespacho = 'Planta Externa';
            $plantaExterna = $PLANTA_ADO->verPlanta($despacho['ID_PLANTA3'] ?? null);
            $destino = $plantaExterna ? ($plantaExterna[0]['NOMBRE_PLANTA'] ?? '') : '';
            break;
        case '6':
            $tipoDespacho = 'Despacho a Productor';
            $productor = $PRODUCTOR_ADO->verProductor($despacho['ID_PRODUCTOR'] ?? null);
            $destino = $productor ? ($productor[0]['NOMBRE_PRODUCTOR'] ?? '') : '';
            break;
        default:
